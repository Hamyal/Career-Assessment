...
const PERSONALIZE_WINDOW_MS = 15 * 60 * 1000;
const MAX_PERSONALIZE_PER_IP = 80;
const personalizeHits = new Map<string, { count: number; resetAt: number }>();

export function checkPersonalizeQuestionRateLimit(request: Request): { ok: true } | { ok: false; status: number; body: { error: string } } {
  const ip = getClientIp(request);
  const now = Date.now();
  let entry = personalizeHits.get(ip);
  if (!entry || now > entry.resetAt) {
    entry = { count: 1, resetAt: now + PERSONALIZE_WINDOW_MS };
    personalizeHits.set(ip, entry);
    return { ok: true };
  }
  entry.count += 1;
  if (entry.count > MAX_PERSONALIZE_PER_IP) {
    return {
      ok: false,
      status: 429,
      body: { error: 'Too many requests. Please slow down.' },
