    ...
    fetch('/api/admin/sessions?' + (searchEmail ? `email=${encodeURIComponent(searchEmail)}` : ''), fetchOpts)
      .then(async (res) => {
        if (res.status === 401) {
          ...
          return { ok: true, data: [] as SessionRow[] };
        }
        setAuthenticated(true);
        if (!res.ok) {
          const body = await res.json().catch(() => ({}));
          throw new Error((body?.error as string) || `Failed to load (${res.status})`);
        }
    ...
    <div className={styles.container}>
      ...
      {loading ? (
        ...
        <div className={styles.emptyMsg}>
          <p>{loadError}</p>
          <p>Check that Supabase env (e.g. SUPABASE_SERVICE_ROLE_KEY) is set and tables exist.</p>
          <button type="button" onClick={() => loadSessions()} className={styles.retryBtn}>
            Retry
          </button>
        </div>
      ) : sessions.length === 0 ? (
        <div className={styles.emptyMsg}>
          ...
          <p>
            {searchEmail
              ...
              : 'Complete an assessment and submit to see sessions here.'}
          </p>
        </div>
      ) : (
        ...
        >
          ...
          >
            ...
            <div className={styles.detailBody}>
              ...
              {selectedSession.participant_code && (
                <div className={styles.detailRow}>
                  <span className={styles.detailLabel}>Participant code</span>
                  <span className={styles.detailValue}>{selectedSession.participant_code}</span>
                </div>
              )}
              <div className={styles.detailRow}>
